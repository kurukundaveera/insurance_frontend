...
class SuggestedPolicies extends Component {
  constructor(props) {
    super(props);
    this.state = {
      modal: false,
  ...
  getData = () => {
    ...
    return new Promise((resolve, reject) => {
      axios.get(config.url+'policies/suggestions').then((response)=> {
  ...
  render() {
    ...
    return (
      <div>
        <div >
            <h3>LIST OF SUGGESTED POLICIES</h3>
                ...
                <tbody>
                    {this.state.listPolices.map((item,i)=>{
                        return(
                            ...
                            <td><button  id="btn4"className="btn btn-outline-primary" onClick={()=>this.handlePolicyDetails(item)} data-toggle="modal">DETAILS</button></td>
                            </tr>
        ...
        >
          ...
          <ModalFooter>
            <Button id="btn6" color="primary" onClick={this.handleBuyPolicy}>
              Ok Buy
            </Button>{" "}
...
export default withTranslation() (SuggestedPolicies);
